...
def ensure_vendor_po_indexes() -> None:
    try:
        with get_db_connection() as conn:
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_vendor_po_lines_po_number ON vendor_po_lines(po_number)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_vendor_po_lines_asin ON vendor_po_lines(asin)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_vendor_po_lines_sku ON vendor_po_lines(sku)"
            )
            conn.commit()
    except Exception as exc:
        logger.warning(f"[DBRepo] Failed to ensure vendor_po_lines indexes: {exc}")


def delete_vendor_po_lines_for_po(po_number: str) -> None:
    try:
        with time_block("vendor_po_lines_delete"):
...
def insert_vendor_po_line(
    ...
    try:
        with time_block("vendor_po_lines_insert"):
...
def bulk_insert_vendor_po_lines(rows: List[Tuple[Any, ...]]) -> None:
    """
    rows: list of tuples matching insert_vendor_po_line params order.
    """
    if not rows:
        return
    sql = """
    ...
    """
    try:
        with time_block(f"vendor_po_lines_bulk_insert:{len(rows)}"):
            execute_many_write(sql, rows)
    except Exception as exc:
        logger.error(f"[DBRepo] Failed bulk insert for vendor_po_lines ({len(rows)} rows): {exc}", exc_info=True)
        raise


def get_vendor_po_lines(po_number: str) -> List[Dict[str, Any]]:
    try:
        with time_block("vendor_po_lines_fetch"):
            with get_db_connection() as conn:
                rows = conn.execute(
                    """
...
def get_vendor_po_line_totals(po_numbers: List[str]) -> Dict[str, Dict[str, int]]:
    ...
    try:
        with time_block(f"vendor_po_lines_totals:{len(po_numbers)}"):
            with get_db_connection() as conn:
                rows = conn.execute(
                    f"""
                    SELECT
...
def get_vendor_po_line_totals_for_po(po_number: str) -> Dict[str, int]:
    try:
        with time_block("vendor_po_lines_totals_single"):
            with get_db_connection() as conn:
                row = conn.execute(
                    """
                    SELECT
...
def get_rejected_vendor_po_lines(po_numbers: List[str]) -> List[Dict[str, Any]]:
    ...
    qmarks = ",".join(["?"] * len(po_numbers))
    try:
        with time_block(f"vendor_po_lines_rejected:{len(po_numbers)}"):
            with get_db_connection() as conn:
                rows = conn.execute(
                    f"""
...
def list_indexes_for_table(table_name: str) -> List[Dict[str, Any]]:
    """
    ...
    try:
        with get_db_connection() as conn:
            ...
            for row in rows:
                results.append({"table": table_name, "name": row["name"], "unique": bool(row["unique"])})
    except Exception as exc:
        logger.debug(f"[DBRepo] Could not list indexes for {table_name}: {exc}")
    return results
